...
const outputName=args.includes('--output-name')?get('--output-name'):'diagnostic-hypotheses';if(!/^[A-Za-z0-9_-]+$/.test(outputName))throw Error('Invalid output name');
const output=join(directory,outputName);if(existsSync(output))throw Error('Preserve prior diagnostic output');mkdirSync(output);
...
await Promise.all(Array.from({length:3},async()=>{while(cursor<tasks.length){const j=tasks[cursor++];let result;
 ...
 const warnings=[];const primary=categories.includes(parsed.primary)?parsed.primary:'uncertain';if(primary!==parsed.primary)warnings.push('Invalid primary category; retained as uncertain');
 const confidence=typeof parsed.confidence==='number'&&parsed.confidence>=0&&parsed.confidence<=1?parsed.confidence:null;if(confidence===null)warnings.push('Invalid confidence; no confidence reported');
 const quotes=Array.isArray(parsed.evidence_quotes)?parsed.evidence_quotes:[];const verified=quotes.filter(x=>typeof x==='string'&&x.length>0&&memories.some(m=>m.content.includes(x)));const rejected=quotes.filter(x=>!verified.includes(x));if(rejected.length)warnings.push('Rejected quote proposals are not source evidence');
 result={qid:j.qid,status:'hypothesis',primary,secondary:Array.isArray(parsed.secondary)?parsed.secondary.filter(x=>categories.includes(x)):[],confidence,reason:String(parsed.reason??''),evidence_quotes:verified,rejected_quote_proposals:rejected,schema_warnings:warnings,raw_model_output:raw};
...
writeFileSync(join(output,'manifest.json'),JSON.stringify({source_run:runId,model,endpoint:process.env.MEMORY_LLM_BASE_URL,prompt_sha256:sha(prompt),script_sha256:scriptHash,output_name:outputName,dataset_sha256:sha(dataset),judgments_sha256:sha(readFileSync(join(directory,'judgments.jsonl'))),created_at:new Date().toISOString()},null,2));
