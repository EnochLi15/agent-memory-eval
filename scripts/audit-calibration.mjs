...
const manifest={protocol:CALIBRATION_PROTOCOL,input_sha256:sha(bytes),input_provenance:Array.isArray(parsed)?null:parsed.provenance??null,prompt_sha256:sha(CALIBRATION_PROMPT),source_sha256:sha(readFileSync(new URL('../src/calibration.ts',import.meta.url))),model_runtime_sha256:sha(readFileSync(new URL('../src/models.ts',import.meta.url))),runner_sha256:sha(readFileSync(new URL(import.meta.url))),model:connection.model,base:connection.base,inference:{stream:true,max_completion_tokens:6000,transport_attempts:3,semantic_retries:0},planned:cases.length,concurrency:2,max_consecutive_transport_failures:4,started_at:new Date().toISOString(),status:'running',scope:'Posthoc model criterion audit. Neither independent human ground truth nor a replacement for historical benchmark scores.'};
...
await Promise.all(Array.from({length:2},async()=>{
 while(cursor<cases.length&&!paused){const c=cases[cursor++];const start=performance.now();let row;
  try{row={qid:c.qid,evaluation_type:c.evaluation_type,...await auditCase(c,connection)};}
...
const summary={protocol:CALIBRATION_PROTOCOL,planned:cases.length,status_counts:count('status'),correct:rows.filter(r=>r.correct===true).length,incorrect:rows.filter(r=>r.correct===false).length,controls:controls.length,control_matches:controls.filter(r=>r.control_match).length,by_type:Object.fromEntries([...new Set(rows.map(r=>r.evaluation_type))].map(type=>[type,{planned:rows.filter(r=>r.evaluation_type===type).length,judged:rows.filter(r=>r.evaluation_type===type&&r.status==='judged').length,control_matches:rows.filter(r=>r.evaluation_type===type&&r.control_match).length}])),scope:manifest.scope};
