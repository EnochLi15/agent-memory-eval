...
Reports distinguish recorded question statuses and questions with no terminal record. Missing retrieval coverage no longer implies that the dataset lacks source annotations. New run manifests record each sample's declared benchmark/background group. `group_outcomes` resamples complete declared groups with all operation variants together; its interval is unavailable for missing metadata, unrecorded questions, or fewer than two groups. It describes recorded outcomes with infrastructure failures in the denominator, not unknown answer correctness. `sample_bootstrap_95` remains explicitly labeled as the legacy sample-ID statistic for historical compatibility; use `group_outcomes` for background-level analysis. Historical artifacts are not rewritten.
