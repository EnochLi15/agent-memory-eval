...
export async function contract(base:string):Promise<void>{
 ...
 const request={request_id:'initial',user_id:id,session_id:'session',messages:[{role:'user',content:'My manager is Clara. My access code is QC-9182.',timestamp:'2026-01-01T00:00:00Z'}]};
 ...
 console.log(JSON.stringify({contract:'passed',base_url:base,user_id:id,checks:['health','echo','idempotency','read-after-write','tenant-isolation','top-k','three-routes','validation','conflict','forget-all-paths','retained-neighbor']}));
}
