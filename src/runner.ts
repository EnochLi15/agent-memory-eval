...
export async function run(config:EvalConfig):Promise<void>{
  ...
  const manifest={run_id:config.runId,mode:config.mode,service_commit:git('../service'),eval_commit:git('.'),workspace_commit:git('..'),dataset_sha256:sha(readFileSync(config.dataFile)),samples:selected.map(s=>s.sample_id),planned_questions:jobs,answer_model:config.answerModel,judge_model:config.judgeModel,judge_kind:config.judgeKind,answer_prompt_sha256:sha(ANSWER_PROMPT),answer_base:config.llmBase,judge_base:config.judgeBase,contract_sha256:sha(readFileSync(new URL('../contracts/contract.json',import.meta.url))),split_sha256:existsSync('configs/splits.json')?sha(readFileSync('configs/splits.json')):null,seed:20260905,hardware:{platform:platform(),arch:arch(),cpu:cpus()[0]?.model,memory_bytes:totalmem()},runtime:process.version,service_config_sha256:process.env.SERVICE_CONFIG_SHA256??null,rubric_prompt_sha256:sha(RUBRIC_PROMPT),top_k:config.topK,chunk_messages:config.maxMessages,chunk_words:config.maxWords,started_at:new Date().toISOString(),conversion:'speaker-preserving-v1;memops-synthetic-order-only',status:'running'};
  if(config.resume&&existsSync(file('manifest.json'))){
    const priorManifest=JSON.parse(readFileSync(file('manifest.json'),'utf8'));
    for(const key of ['run_id','mode','dataset_sha256','planned_questions','answer_model','judge_model','judge_kind','answer_prompt_sha256','rubric_prompt_sha256','top_k','chunk_messages','chunk_words','answer_base','judge_base','contract_sha256','service_config_sha256'] as const){if(priorManifest[key]!==manifest[key])throw new Error(`Resume configuration changed: ${key}`);}
  }
  ...
  await Promise.all(Array.from({length:Math.max(1,config.concurrency)},async()=>{
    while(cursor<selected.length){const sample=selected[cursor++]!;const userId=`${config.runId}:${sample.benchmark}:${sample.sample_id}`;let failed:string|null=null;
      for(const session of sample.sessions){if(failed)break;for(const [i,messages] of chunks(session.messages,config.maxMessages,config.maxWords).entries()){
        const request_id=`${userId}:${session.session_id}:${i}`;if(ingested.has(request_id))continue;const request={request_id,user_id:userId,session_id:session.session_id,messages};const start=performance.now();
        try{log('requests.jsonl',{path:'/add',body:request});await client.add(request);log('ingest.jsonl',{request_id,sample_id:sample.sample_id,status:'ok',elapsed_ms:performance.now()-start,message_count:messages.length});ingested.add(request_id);}
        catch(e){failed=String(e);log('ingest.jsonl',{request_id,status:'failed',error:failed,elapsed_ms:performance.now()-start});break;}
      }}
      for(const q of sample.questions){if(done.has(q.qid))continue;const common={qid:q.qid,sample_id:sample.sample_id,benchmark:sample.benchmark,category:q.category};if(failed){log('judgments.jsonl',{...common,status:'service_error',error:failed});continue;}
        try{
          const searchRequest={query:q.question,user_id:userId,top_k:config.topK,...(q.options?{options:q.options}:{})};log('requests.jsonl',{path:'/search',body:searchRequest});const start=performance.now();const memories=await client.search(searchRequest);log('retrievals.jsonl',{...common,memories,elapsed_ms:performance.now()-start});
