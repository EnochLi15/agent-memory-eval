...
export function memops(directory:string,filesLimit=0):Sample[]{
  ...
  return names.slice(0,filesLimit||names.length).map(name=>{
    ...
    return {sample_id:name.replace('.json',''),group_id:name.split('_')[0]!,benchmark:'memops',
      sessions:(d.conversations as Obj[]).map((s,i)=>({session_id:`segment-${s.segment_index??i}`,messages:(s.dialogue as Obj[]).map((m,j)=>({role:String(m.role),content:String(m.content),timestamp:new Date(Date.UTC(2025,0,1)+i*86400000+j*1000).toISOString()}))})),
...
export function competition(path:string):Sample[]{
 ...
 return data.map(s=>({sample_id:String(s.sample_id),group_id:String(s.sample_id),benchmark:s.benchmark==='locomo'?'locomo':'memops',sessions:s.add_phase.sessions.map((x:Obj,i:number)=>({session_id:x.session_id??`session-${i}`,messages:x.messages})),questions:s.search_items.map((q:Obj)=>({qid:q.qid,question:q.question,gold_answer:q.gold_answer,gold_rubric:q.gold_rubric,options:q.options,category:q.category??'unknown'}))}));
}
